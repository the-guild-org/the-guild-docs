...
export const SearchButton = styled.button(({ accentColor, isFull }: IStyleProps) => [
  ...
  css`
    img {
      ${[tw`h-6 w-6 m-0 md:(h-4.5 w-4.5 mr-1) hover:opacity-70`, tw`transition duration-200 ease-in-out`]}
    }
...
export const SearchForm = styled.div(({ accentColor }: IStyleProps) => [
  ...
  css`
    ...
    button {
      ${[tw`p-0 bg-transparent border-0 cursor-pointer`, tw`transition duration-200 ease-in-out`, tw`hover:(opacity-70)`]}
    }
  `,
]);
...
export const SearchHit = styled.article(({ accentColor }: IStyleProps) => [
  css`
    ...
    a {
      ${[tw`flex items-center mb-2 px-5 py-3 rounded-md break-all no-underline`, tw`dark:bg-gray-800 bg-gray-100`]}
