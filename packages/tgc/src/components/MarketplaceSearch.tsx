...
import { Container, Results, Search, Title, Wrapper } from './MarketplaceSearch.styles';
...
export const MarketplaceSearch: React.FC<IMarketplaceSearchProps> = ({
  ...
  const renderQueryPlaceholder = (placeholder: string | React.ReactElement, query: string | undefined) => {
  ...
  useEffect(() => {
    let results = null;
    if (query) {
      results = queryList.items.filter(item => item.title.toLowerCase().includes(query.toLowerCase()));
    }
  ...
  return (
    <Wrapper>
      <Container>
        <Title>{title}</Title>
        <Search>
          <img src={marketplaceAssets.search} alt="Search" height="24" width="24" />
