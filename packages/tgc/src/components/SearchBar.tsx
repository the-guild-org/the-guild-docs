...
import { InstantSearch, Highlight, connectHits, connectSearchBox, connectStateResults } from 'react-instantsearch-dom';
...
import { SearchButton, SearchForm, SearchResults, SearchHit } from './SearchBar.styles';
...
const Hits: React.FC<{ hits: Hit<any>[]; accentColor: string }> = ({ hits, accentColor }) => {
  const icons = useIcons();
...
export const SearchBar: React.FC<ISearchBarProps> = ({ accentColor, title, placeholder, isFull, onHandleModal }) => {
  ...
  return (
    <>
      <SearchButton accentColor={accentColor} isFull={isFull || false} onClick={() => handleModal(true)}>
